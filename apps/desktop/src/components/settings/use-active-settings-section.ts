...
/**
 * The reading line: a section is active while it is the last one whose top has
 * crossed this many pixels below the container's top edge. Sits just under the
 * jump offset so clicking a navigator entry always lands with that entry
 * active.
 */
 ...
 * rendered inside it — and recomputes on scroll and resize. The active section
 * is the last one whose top has crossed the reading line; once the container is
 * scrolled to the very bottom the final section wins instead, since a short
 * last card may never reach the line on its own. (Adapted from Reflect Open.)
 */
