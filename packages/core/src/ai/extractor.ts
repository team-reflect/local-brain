...
- "projects": { ref, name }
- "tasks": { ref, title, description?, status?, dueAt?, scheduledFor?, projectRef?, assigneeRefs?, personRefs?, evidence? }
...
- Task "status" is one of: open, in_progress, waiting, blocked, done, cancelled.
  Scheduling belongs in "scheduledFor"; never use "scheduled" as a status.
- For tasks, use "assigneeRefs" for the person(s) responsible for doing the task (e.g.
  "Dana should send the deck" → assigneeRefs: [dana_ref]). Use "personRefs" for people merely
