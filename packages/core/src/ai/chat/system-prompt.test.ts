...
describe('buildChatSystemPrompt', () => {
  ...
  it('directs efficient structured retrieval instead of search loops', () => {
    ...
    expect(prompt).toContain('12 results by default, 16 maximum')
    expect(prompt).toContain('hard budget of two discovery calls')
    ...
    expect(prompt).toContain('batch at most the six most promising records into its single detail call')
    expect(prompt).toContain('at most four model steps')
    expect(prompt).toContain('After get_records returns')
