...
export function buildChatSystemPrompt({ today, projects }: ChatSystemPromptInput): string {
  const lines: string[] = [
    ...
    '',
    'Grounding rules:',
    '- Use search_records to look up relevant local records before answering topic questions.',
    '- search_records supports hybrid semantic search when you pass a query, so use natural topic wording even when the source wording may differ.',
    '- Use search_records filters when the user names traits such as record type, dates, source slugs, email/meeting kind, transcript presence, task status, or linked record ids. Omit query for metadata-only requests like recent emails or recent transcripts.',
    ...
    '- Before creating tasks from recent emails, meetings, or transcripts, search_records with filters first, then create only concrete follow-ups grounded in returned excerpts.',
