...
export function buildChatSystemPrompt({ today, projects }: ChatSystemPromptInput): string {
  const lines: string[] = [
    ...
    '',
    'Grounding rules:',
    '- Use search_records to discover candidate documents, interactions, transcripts, emails, tasks, people, and other records. Pass a query for topic search; add recordTypes, kinds, after/before, or sort to filter by record type, interaction kind (e.g. email), or date. Hybrid search is the default: it combines lexical keyword search with semantic vector recall. Use mode:"semantic" for semantic-only recall when exact terms may not appear; use mode:"lexical" only when exact keywords, IDs, or quoted text should stay strictly lexical.',
    '- Use get_records after search_records to load structured details and bounded grounding chunks for promising hits before answering detail-heavy questions. Pass recordType, recordId, and any relevant chunkIds returned by search_records.',
