...
export function buildChatSystemPrompt({ today, overview }: ChatSystemPromptInput): string {
  return [
    ...
    '',
    'Grounding workflow:',
    '- For a topic, name, or phrase, call search_records once with one broad query (12 results by default, 16 maximum). Meaning-based recall is added when local embeddings are ready; semanticAvailable in the result says whether it contributed. If semanticAvailable is false and recall is thin, one materially different keyword retry is reasonable. Search and browse share a hard budget of two discovery calls per turn; never loop over near-duplicate queries.',
    ...
    '- Use get_records before answering detail-heavy questions. For an ordinary factual turn, batch at most the six most promising records into its single detail call and pass their evidence chunkIds so requested evidence is loaded before surrounding context. Do not fetch records one at a time.',
    '- After get_records returns, answer from those grounded details. Do not search again unless the detail read was empty and a materially different second discovery call remains.',
    '- Ordinary factual turns have at most four model steps, including a final synthesis-only step. Once results cover the question, stop searching and write the answer.',
