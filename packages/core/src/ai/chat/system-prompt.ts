...
export function buildChatSystemPrompt({ today, projects }: ChatSystemPromptInput): string {
  const lines: string[] = [
    ...
    '',
    'Grounding rules:',
    '- Use search_records to discover candidate documents, interactions, transcripts, emails, tasks, people, and other records. Pass a query for topic search; add recordTypes, kinds, after/before, or sort to filter by record type, interaction kind (e.g. email), or date. Hybrid search is the default; use mode:"lexical" only when exact keywords, IDs, or quoted text should stay strictly lexical.',
