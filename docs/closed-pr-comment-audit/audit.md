...
| 13 | 19 | Wrong model status copy | Med | `apps/desktop/src/components/first-run.tsx` | **fixed now** | First-run used `canRun` for "configured"; it now uses `configured`, and the old separate external-call toggle has since been removed. |
